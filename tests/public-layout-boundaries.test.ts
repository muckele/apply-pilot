...
test("public layout owns the exact public pages, scoped CSS, and shell without AppShell", () => {
  const publicLayoutPath = "app/(public)/layout.tsx";

  assert.equal(existsSync(absolutePath(publicLayoutPath)), true);
  assert.equal(existsSync(absolutePath("app/(public)/public.css")), true);
  ...
  assert.equal(existsSync(absolutePath("app/(public)/signup/page.tsx")), true);
  ...
  assert.match(source, /import "\.\/public\.css";/);
  assert.match(source, /className="public-shell"/);
  assert.doesNotMatch(source, /@\/components\/app-shell/);
  assert.doesNotMatch(source, /<AppShell\b/);
});
...
test("product layout owns AppShell and every current-main authenticated product page", () => {
...
test("the complete current-main API inventory remains outside route groups", () => {
