...
const stages = [
  ...
  {
    title: "Evaluate",
    copy: "Review an illustrative Apply Pilot job-fit estimate and its reasoning.",
    icon: Check,
    visual: "reasoning"
  },
  {
    title: "Prepare",
    copy: "Prepare application material for your review.",
    icon: FileText,
    visual: "evidence"
  },
  {
    title: "Review",
    copy: "Review and revise every generated or saved material before use.",
    icon: ShieldCheck,
    visual: "review"
  },
  {
    title: "You submit",
    copy: "Take your reviewed materials to the employer site, then personally submit.",
    icon: UserRound,
    visual: "browser"
  }
] as const;
  ...
  if (type === "reasoning") {
    return (
      <div className="workflow-mini reasoning-mini">
        <div><strong>Fit reasoning</strong><span>Illustrative strong fit</span></div>
        {['Cross-functional ops', 'Process design', 'Operational impact'].map((label) => (
          <p key={label}><Check aria-hidden="true" /> <span><strong>{label}</strong><small>Illustrative experience</small></span></p>
        ))}
      </div>
    );
  }
  if (type === "evidence") {
    return (
      <div className="workflow-mini evidence-mini">
        <strong>Experience map</strong>
        {['Cross-functional initiative', 'Process design', 'Impact metrics', 'Stakeholder alignment'].map((label) => (
          <p key={label}><i aria-hidden="true" /><span>{label}<small>Example profile information</small></span></p>
        ))}
      </div>
    );
  }
  if (type === "review") {
    return (
      <div className="workflow-mini review-mini">
        <div><strong>Material review</strong><span>1 of 6</span></div>
        <small>Question</small>
        <p>Describe a time you improved an operational process.</p>
        <div><span>Edit</span><span>Ready</span></div>
      </div>
    );
  }
  return (
    <div className="workflow-mini browser-mini">
      <div><strong>Application checklist</strong><span><i /> Ready</span></div>
      <div className="browser-chrome"><i /><i /><i /><span /></div>
      <div className="browser-form"><i /><i /><b /></div>
      <p><ShieldCheck aria-hidden="true" /> You enter and submit.<br />Always in control.</p>
    </div>
  );
}
