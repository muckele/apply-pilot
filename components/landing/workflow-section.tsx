...
const stages = [
  ...
  {
    title: "Prepare",
    copy: "Build application material from source-linked experience.",
    icon: FileText,
    visual: "evidence"
  },
  {
    ...
    return (
      <div className="workflow-mini reasoning-mini">
        ...
        {['Cross-functional ops', 'Process design', 'Operational impact'].map((label) => (
          <p key={label}><Check aria-hidden="true" /> <span><strong>{label}</strong><small>Source-linked experience</small></span></p>
        ))}
      </div>
    );
  }
    ...
    return (
      <div className="workflow-mini evidence-mini">
        ...
        {['Cross-functional initiative', 'Process design', 'Impact metrics', 'Stakeholder alignment'].map((label) => (
          <p key={label}><i aria-hidden="true" /><span>{label}<small>Provided source</small></span></p>
        ))}
      </div>
    );
  }
