...
- The authoritative starting point is `bae9a8036c5f3250c54b556dcb649c697658853e` on `feature/form-inspection-answer-packet`; the approved design SHA-256 is `78b1042e2aafa541188505761cfaa9ca488ff7fa0d5d99c61d9ded7311bb7409`.
...
- A refreshed packet replaces the prior packet as one unit. Old answer objects, IDs, versions, closures, and detached buttons cannot start a request.
...
Task 1 introduces these exact exported interfaces in `lib/application-browser/control-presentation.ts`:
...
`fetchReviewAuthority(expectedGeneration)` starts both same-origin GETs with one abort signal, parses both responses, checks both identities against the immutable `runId`, and commits `{ run, packet }` once only after both reads pass. Separate endpoints can race with server mutations, so the UI treats the resulting values as a request snapshot while the POST routes retain the final exact-version/hash authority and return `409` on any mismatch.

## Failure and Lifecycle Contract

| Outcome | Local data | Controls | Follow-on behavior | Notice |
|---|---|---|---|---|
| Valid mutation response and valid run+packet refresh | Replace run and packet atomically | Recomputed from refreshed values | None beyond the one refresh | Success only after refresh |
...
- Produces: `ReviewRunAuthority`, `PendingReviewMutation`, `SameOriginReviewRequest`, `isAnswerReviewEligible`, `isResolveReviewEligible`, `parseApplicationRunReviewResponse`, `buildAnswerReviewRequest`, `parseAnswerReviewResponse`, and `buildResolveReviewRequest` with the signatures in “Planned Interfaces.”
...
- [ ] **Step 4: Implement the strict schemas and pure helpers.**
  ...
  Use a closed list of all current `ApplicationRunState` values and `z.enum(PLAN_REVIEW_REASONS)`. Refine `reviewReasons` so each reason is unique and its index in `PLAN_REVIEW_REASONS` strictly increases. Parse the current run DTO through a strict top-level `{ run }` schema while allowing only the documented `ApplicationRunDto` fields inside `run`; project only `id`, `state`, `stateVersion`, and a cloned `reviewReasons` array. Define the answer mutation response as strict `{ answer: { id, runId, status, reviewedByUser, reviewedAt, sensitive, valueRedacted } }` and compare its identity/status to the request snapshot. Build request objects from scalar inputs only. Return resolve eligibility false for already reviewed packets and every state other than `REVIEW_REQUIRED`.
...
- [ ] **Step 5: Run the focused test and verify GREEN.**
  ...
  Expected: PASS, including the new eligibility, request-body, run-parser, response-parser, and resolve-contract cases.
...
- Produces: component-local `ReviewLoad`, `fetchReviewAuthority(expectedGeneration)`, `invalidateReviewAuthority(...)`, and `reviewAnswer(answer, status)` behavior; no new exported production API.
...
- [ ] **Step 2: Add RED authoritative refresh, failure, replacement, lifecycle, and binding-privacy tests.**
  ...
  1. A valid answer-review response triggers exactly one new run GET and one new packet GET; only the refreshed packet changes the displayed status/counts and produces the success notice.
  2. Before that refresh settles, the old packet remains displayed but all review controls are inert.
  3. `409` marks old review data untrusted before starting one run+packet refresh; a held refresh proves the old button is disabled, and a successful replacement removes the old answer action.
  4. Retain an old answer button reference, complete a packet replacement with a new packet/version/answer ID, click the detached old button, and assert no POST occurs.
  5. `401`, `403`, and `404` clear run/packet review authority and render the bounded corresponding notice.
  6. `429`, representative `500` and `503`, and a network rejection preserve the displayed packet text but mark it unverified and disable all review mutations without changing displayed answer status.
  7. A `200` whose `json()` rejects, wrong-run answer DTO, wrong-answer DTO, wrong-status DTO, or strict-parser-invalid DTO produces no success notice, no local status mutation, and inert review data.
  8. Unmount before a valid or rejected answer mutation settles; settlement performs no state update and adds no run GET, packet GET, resolve POST, recovery action, notice, or binding invocation.
  9. Install a binding spy, complete approve and reject flows, and assert it receives zero commands. Serialize all binding inputs and assert absence of the answer ID, proposal sentinel, packet hash, `APPROVED`, `REJECTED`, and review request URL.
...
- [ ] **Step 4: Replace packet-only loading with atomic review-authority loading.**
  ...
  Rename the component state/ref from `PacketLoad`/`packetLoadRef` to `ReviewLoad`/`reviewLoadRef`. Replace `fetchPacket` with `fetchReviewAuthority`; make it issue the owned-run GET and packet GET under one component generation, request sequence, and abort controller. Check generation/sequence/abort after each awaited boundary. Parse both values before one state/ref update. Preserve the current automatic refresh key behavior by calling the new combined read after successful inspection status.
...
- [ ] **Step 5: Implement globally serialized answer mutations without optimistic authority.**
  ...
  After the POST, verify the component generation before parsing or changing refs. On valid success, call `fetchReviewAuthority(expectedGeneration)` and show success only if that read committed. On `409`, mark current review data unverified and begin one combined refresh. On every other failure, apply the table exactly. The answer objects and summary remain unchanged until a valid combined read replaces them.
...
**Interfaces:**
- Consumes: `isResolveReviewEligible`, `buildResolveReviewRequest`, `parseApplicationRunReviewResponse`, the Task 2 `ReviewLoad`, `fetchReviewAuthority`, mutation lock/ref, generation checks, and invalidation behavior.
...
- [ ] **Step 1: Add RED resolve visibility, exact-request, and success-refresh tests.**
  ...
  - a valid resolve response triggers exactly one combined run+packet refresh; only refreshed run state and packet `reviewedAt` remove resolution availability and produce the success notice.
...
- [ ] **Step 2: Add RED stale/failure/unmount/packet-replacement resolve tests.**
  ...
  - if a packet/run refresh replaces the exact version/hash/state/reasons before a retained old resolve button is clicked, no POST is sent;
...
- [ ] **Step 4: Implement resolve review through the shared mutation surface.**
  ...
  Render current run state/version and the ordered current review reasons in the packet summary area so the user can see the authority being acknowledged. Use bounded human-readable labels for the six closed reason identifiers while preserving the raw ordered identifiers only in the request snapshot. When the reason array is empty, render `No planner review reasons require acknowledgment.`

  Add `resolveReview()` using the same pending ref, generation snapshot, abort controller, response-status mapping, parser, and combined refresh as `reviewAnswer`. Before dispatch, reread `reviewLoadRef.current`, require `isResolveReviewEligible(...)`, and build from those exact current objects. The active label is `Resolving…`; all answer and resolve controls are disabled for the duration. A valid response is only a trigger for the combined authoritative refresh and never directly updates displayed run state.
...
- [ ] **Step 6: Run the focused test and verify GREEN.**
  ...
  Expected: PASS for all 22 required behavior groups plus the repository-derived run-state, reviewed-packet, noncanonical-reason, and retained-old-control cases.
...
- [ ] **Step 10: Prove scope and binding privacy from the final diff.**
  ...
  git diff HEAD~3 -- components/application-browser-control.tsx lib/application-browser/control-presentation.ts tests/application-browser-control-presentation.test.ts
  ...
  Expected: the name list contains exactly the two production files and one test file in this plan. The review confirms every HTTP review action remains outside the binding. The ripgrep command exits `1` with no matches.
...
| 9 | Successful answer review | 2 | Starts one authoritative run+packet refresh; only refreshed data changes UI |
...
| 17 | Packet replacement | 2/3 | Old answer/version/hash controls cannot dispatch |
...
| 20 | Successful resolve | 3 | Starts combined refresh; refreshed authority removes availability |
...
| 26 | Old detached button | 2/3 | Retained DOM reference after authoritative replacement sends no request |
...
- Every successful mutation performs an authoritative run+packet refresh before the UI presents success.
...
| Server-authoritative refresh | Task 2 combined loader and both mutation success suites |
...
| Packet replacement | Atomic `ReviewLoad`, current-object dispatch check, old-control tests |
...
- **Type/interface consistency:** The same `ReviewRunAuthority`, `PendingReviewMutation`, `SameOriginReviewRequest`, parser, predicate, and builder names/signatures are used in all tasks and tests. Request property names exactly match the current server schemas.
