...
The authenticated backend includes controlled preparation, deterministic review, guarded one-attempt Fill, cancellation, audit, and execution-token foundations. `PREPARE_ONLY` remains the default policy mode. Fill additionally requires the opt-in `FILL_AND_REVIEW` mode, the per-user policy to be enabled, the global `APPLICATION_AUTOMATION_ENABLED` gate, and the existing reviewed host/run authority. `APPLICATION_READ` remains the only execution-token scope that can currently be issued; the browser workflow uses the authenticated owner session rather than issuing a Fill token.

The headed local companion can explicitly inspect the frozen anonymous employer form, publish or replay the correlated inspection, and let the authenticated run-owned page review the current answer packet. Once that packet is reviewed and all trusted state agrees, the user may explicitly choose **Fill approved fields**. One guarded attempt can fill the reviewed supported `TEXT`, `EMAIL`, `TEL`, `URL`, `TEXTAREA`, and single-select fields. Occupied writable controls are preserved. Radio buttons, checkboxes, manual-only, excluded, unsupported, rejected, and runtime-ineligible fields remain manual. Packet and proposal material never crosses the control-page bridge.

Fill is a Human-Submit workflow, not auto-apply: Apply Pilot does not upload documents, click employer controls, or submit the application. After Fill stops or finishes, the user reviews the employer page, completes every remaining manual field, confirms preserved values, and personally clicks the employer site's Submit button. There is no automatic Fill retry or recovery button. The manual browser capture extension remains a separate review-before-save workflow. See [docs/CONTROLLED_APPLICATION_AUTOMATION.md](docs/CONTROLLED_APPLICATION_AUTOMATION.md) for the current capability and [docs/APPLICATION_BROWSER_COMPANION.md](docs/APPLICATION_BROWSER_COMPANION.md) for launch, privacy, one-attempt, reconciliation, and safe-stop behavior.
