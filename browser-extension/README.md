...
4. In Apply Pilot, open **Settings > Integrations**, generate a browser token, and copy the app URL.
5. Open the extension’s **Setup** tab and enter both values.

The app URL is stored in `chrome.storage.local`. The scoped browser token is stored in `chrome.storage.session`, so it is cleared when the browser session ends. Revoke tokens from Apply Pilot at any time.
