...
function normalizedAppUrl(value) {
  ...
  if (url.protocol !== "https:" && !localHost) throw new Error("Use HTTPS unless Apply Pilot is running on this computer.");
  return url.origin;
}
...
async function saveJob(event) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(json?.error || "Apply Pilot rejected the capture.");
...
async function saveSettings() {
  ...
  if (!token.startsWith("jmc_")) throw new Error("Paste a browser token generated by Apply Pilot.");

  const granted = await chrome.permissions.request({ origins: [`${appUrl}/*`] });
  if (!granted) throw new Error("Chrome needs permission to connect only to your Apply Pilot origin.");
